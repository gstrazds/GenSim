...
class ColorCoordinatedCylinderOnBlock(Task):
    """Pick up each colored cylinder and place it onto the block of the same color from top."""

    def __init__(self):
        super().__init__()
        self.max_steps = 20
        self.lang_template = "put the {} cylinder on the {} block"
        self.task_completed_desc = "done placing cylinders on blocks."
    ...
    def reset(self, env):
        ...
        # Add cylinders.
        cylinder_size = (0.04, 0.04, 0.10)
        cylinder_urdf = 'cylinder/rod.urdf'
        cylinders = []
        for color in self.colors:
